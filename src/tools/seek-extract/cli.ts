import { seekExtract } from './index.js';
...
export async function runCli(argv: string[]): Promise<void> {
  ...
  if (!args.url) {
    console.error('Usage: career-ops seek-extract <seek-url> [--no-llm] [--no-store] [--reextract] [--json]');
    process.exit(2);
  }

  const job = await seekExtract(args.url, {
    noLlm: args.noLlm,
