...
import type { SeekJob } from '../../shared/db/types.js';
...
async function extractOnPage(page: Page, url: string, opts: ExtractOptions): Promise<SeekJob> {
...
export async function seekExtract(url: string, opts: ExtractOptions = {}): Promise<SeekJob> {
  const jobId = extractJobIdFromUrl(url);
