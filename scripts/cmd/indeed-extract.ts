import { indeedExtract } from '../../src/tools/indeed-extract/index.js';
...
export async function runCli(argv: string[]): Promise<void> {
  ...
  if (!args.url) {
    console.error('Usage: career-ops indeed-extract <indeed-url> [--no-llm] [--no-store] [--reextract] [--json]');
    process.exit(2);
  }

  const job = await indeedExtract(args.url, {
    noLlm: args.noLlm,
