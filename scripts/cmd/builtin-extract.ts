import { builtinExtract } from '../../src/tools/builtin-extract/index.js';
...
export async function runCli(argv: string[]): Promise<void> {
  ...
  if (!args.url) {
    console.error('Usage: career-ops builtin-extract <builtin-url> [--no-llm] [--no-store] [--reextract] [--json]');
    process.exit(2);
  }

  const job = await builtinExtract(args.url, {
    noLlm: args.noLlm,
